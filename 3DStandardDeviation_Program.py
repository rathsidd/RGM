...
class Adj_Mats(object):
    ...
    def get_atom_dists(self):
        ...
        base = np.zeros((len(frameindices), len(frames[0]), 3))
        #i=framecount
        for i in range(len(frameindices)):
            for j in range(len(frames[i])):
                for k in range(len(frames[i][j])):
                    base[i][j][k] = frames[frameindices[i]][j][k]
        dists = np.reshape(base, (len(frameindices), 1, len(frames[0]), 3)) - np.reshape(base, (len(frameindices), len(frames[0]), 1, 3))
        dists = dists**2
        dists = dists.sum(3)
        dists = np.sqrt(dists)
    ...
    def get_elec_adj(self):
        ...
        self.elec_adjacency_graphs = np.zeros((len(frameindices), total_val, total_val))
    ...
    def make_eigenvalues(self, hamiltonian_iter=2):
        self.elec_adjacency_graphs=[]
        self.elec_adjacency_graphs.append(self.get_elec_adj())
...
for upperlimit in upperlimit_list:
    if __name__ == "__main__":
        ...
        results=full.get_spacings_and_stdevs(types,frameindices)
        if (types=='all'):
    ...
    hydrogenbonds_allframes.append(hydrogenbonds_array)
